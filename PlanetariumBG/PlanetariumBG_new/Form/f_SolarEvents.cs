    ...
    public class f_SolarEvents : System.Windows.Forms.Form
	{
		...
		private void SearchFunc()
		{
			...
			if (r_SolarEclipse.Checked == true){
				...
				dateList.Clear();
				NoEcl = 0;
				location.v_mainDT = new DateTime((int)nUD_From.Value, 1, 1);
				location.v_mainDT = location.v_mainDT.AddDays(-1);
				planetData.PlanetPositions();
				oldElong = planetData.planets["Moon"].elong;
				step = "day";
				while (location.v_mainDT.Year < (int)nUD_To.Value+1){
					prog = ((location.v_mainDT.Year-(int)nUD_From.Value)*100)/((int)nUD_To.Value-(int)nUD_From.Value);
					l_Progress.Invalidate();
					AddTime (step, 1);
					planetData.PlanetPositions();
					newElong = planetData.planets["Moon"].elong;
					...
					if(step=="minute" && newElong>oldElong){
						if(oldElong<(planetData.planets["Moon"].diam/7200+planetData.planets["Sun"].diam/7200)){
							AddTime(step,-1);
							++NoEcl;
							lB_Result.Items.Add(NoEcl+".   "+location.v_mainDT);
							DateTime tempTime = new DateTime();
							tempTime = location.v_mainDT;
			...
			if (r_LunarEclipse.Checked == true)
			{
				...
				while (location.v_mainDT.Year < (int)nUD_To.Value+1)
				{
					...
					if(step=="minute" && newElong<oldElong)
					{
						...
						if(oldElong>180-(planetData.planets["Moon"].diam/7200+planetData.planets["Earth shadow"].diam/7200))
						{
							AddTime(step,-1);
							++NoEcl;
							lB_Result.Items.Add(NoEcl+".   "+location.v_mainDT);
			...
			if (r_MercuryTranzit.Checked == true){
				...
				location.v_mainDT = location.v_mainDT.AddDays(-1);
				planetData.PlanetPositions();
				oldElong = planetData.planets["Mercury"].elong;
				step = "day";
				while (location.v_mainDT.Year < (int)nUD_To.Value+1){
					prog = ((location.v_mainDT.Year-(int)nUD_From.Value)*100)/((int)nUD_To.Value-(int)nUD_From.Value);
					l_Progress.Invalidate();
					AddTime (step, 1);
					planetData.PlanetPositions();
					newElong = planetData.planets["Mercury"].elong;
					...
					if(step=="minute" && newElong>oldElong){
						if(oldElong<(planetData.planets["Mercury"].diam/7200+planetData.planets["Sun"].diam/7200)){
							if (planetData.planets["Mercury"].dist < planetData.planets["Sun"].dist){
								AddTime(step,-1);
								++NoEcl;
								lB_Result.Items.Add(NoEcl+".  "+location.v_mainDT);
								DateTime tempTime = new DateTime();
			...
			if (r_VenusTranzit.Checked == true)
			{
				...
				location.v_mainDT = location.v_mainDT.AddDays(-1);
				planetData.PlanetPositions();
				oldElong = planetData.planets["Venus"].elong;
				step = "day";
				while (location.v_mainDT.Year < (int)nUD_To.Value+1){
					prog = ((location.v_mainDT.Year-(int)nUD_From.Value)*100)/((int)nUD_To.Value-(int)nUD_From.Value);
					l_Progress.Invalidate();
					AddTime (step, 1);
					planetData.PlanetPositions();
					newElong = planetData.planets["Venus"].elong;
					if(newElong<2) step="hour";
					if(newElong<0.4) step="minute";
					if(step=="hour" && newElong>oldElong){
						step = "day";
					...
					if(step=="minute" && newElong>oldElong){
						if(oldElong<(planetData.planets["Venus"].diam/7200+planetData.planets["Sun"].diam/7200)){
							if (planetData.planets["Venus"].dist < planetData.planets["Sun"].dist){
								AddTime(step,-1);
								++NoEcl;
								lB_Result.Items.Add(NoEcl+".  "+location.v_mainDT);
								DateTime tempTime = new DateTime();
		...
		private void pB_Space_Paint(object sender, System.Windows.Forms.PaintEventArgs e)
		{
			...
			if (r_LunarEclipse.Checked == true)
			{
				planetData.planets["Earth shadow"].skyPosition.eqToaA(location.SIDTIME, location.v_Lat);
				if (planetData.planets["Earth shadow"].skyPosition.a < 0)
					g.DrawString ("Not visible !", new Font("Arial",12),
						new SolidBrush(Color.Red), 10, 10);
				g.DrawString (location.v_mainDT.ToString(), new Font("Arial",10),
				...
				double tempRA = planetData.planets["Earth shadow"].skyPosition.RA-planetData.planets["Moon"].skyPosition.RA;
				if (tempRA > 300)	tempRA-=360;
				double dx = (tempRA*pB_Space.Width/3)/(planetData.planets["Earth shadow"].diam/7200);
				double dy = ((planetData.planets["Earth shadow"].skyPosition.Decl-planetData.planets["Moon"].skyPosition.Decl)*pB_Space.Width/3)/(planetData.planets["Earth shadow"].diam/7200);
				double DMM = (((EarthShadow)planetData.planets["Earth shadow"]).DU*pB_Space.Width/1.5)/((EarthShadow)planetData.planets["Earth shadow"]).DP;
				double DM = (planetData.planets["Moon"].diam*pB_Space.Width/1.5)/planetData.planets["Earth shadow"].diam;
				...
				g.DrawString ("RA : "+Math.Round(planetData.planets["Earth shadow"].skyPosition.RA,3).ToString(), new Font("Arial",10),
					new SolidBrush(Color.DarkGray), pB_Space.Width/2+10, 30);
				g.DrawString ("Decl : "+Math.Round(planetData.planets["Earth shadow"].skyPosition.Decl,3).ToString(), new Font("Arial",10),
					new SolidBrush(Color.DarkGray), 15, pB_Space.Height/2+15);
			}
			else
			{
				planetData.planets["Sun"].skyPosition.eqToaA(location.SIDTIME, location.v_Lat);
				if (planetData.planets["Sun"].skyPosition.a < 0)
					g.DrawString ("Not visible !", new Font("Arial",12),
						new SolidBrush(Color.Red), 10, 10);
				g.DrawString (location.v_mainDT.ToString(), new Font("Arial",10),
					new SolidBrush(Color.White), 10, 30);
				...
				if (r_SolarEclipse.Checked == true)
				{
					...
					double tempRA = planetData.planets["Sun"].skyPosition.RA-planetData.planets["Moon"].skyPosition.RA;
					if (tempRA > 300)	tempRA-=360;
					double dx = (tempRA*pB_Space.Width/8)/
						(planetData.planets["Sun"].diam/7200);
					double dy = ((planetData.planets["Sun"].skyPosition.Decl-planetData.planets["Moon"].skyPosition.Decl)*pB_Space.Width/8)/
						(planetData.planets["Sun"].diam/7200);
					double DM = (planetData.planets["Moon"].diam*pB_Space.Width/4)/planetData.planets["Sun"].diam;
					g.FillEllipse(new SolidBrush(Color.DarkSlateGray),
						pB_Space.Width/2+(int)dx-(int)DM/2, pB_Space.Height/2-(int)dy-(int)DM/2, (int)DM, (int)DM);
				}
				if (r_MercuryTranzit.Checked == true)
				{
					...
					double tempRA = planetData.planets["Sun"].skyPosition.RA-planetData.planets["Mercury"].skyPosition.RA;
					if (tempRA > 300)	tempRA-=360;
					double dx = (tempRA*pB_Space.Width/4)/
						(planetData.planets["Sun"].diam/7200);
					double dy = ((planetData.planets["Sun"].skyPosition.Decl-planetData.planets["Mercury"].skyPosition.Decl)*pB_Space.Width/4)/
						(planetData.planets["Sun"].diam/7200);
					double DM=6;
					g.FillEllipse(new SolidBrush(Color.DarkSlateGray),
						pB_Space.Width/2+(int)dx-(int)DM/2, pB_Space.Height/2-(int)dy-(int)DM/2, (int)DM, (int)DM);
				...
				if (r_VenusTranzit.Checked == true)
				{
					...
					double tempRA = planetData.planets["Sun"].skyPosition.RA-planetData.planets["Venus"].skyPosition.RA;
					if (tempRA > 300)	tempRA-=360;
					double dx = (tempRA*pB_Space.Width/4)/
						(planetData.planets["Sun"].diam/7200);
					double dy = ((planetData.planets["Sun"].skyPosition.Decl-planetData.planets["Venus"].skyPosition.Decl)*pB_Space.Width/4)/
						(planetData.planets["Sun"].diam/7200);
					double DM = (planetData.planets["Venus"].diam*pB_Space.Width/2)/planetData.planets["Sun"].diam;
					g.FillEllipse(new SolidBrush(Color.DarkSlateGray),
						pB_Space.Width/2+(int)dx-(int)DM/2, pB_Space.Height/2-(int)dy-(int)DM/2, (int)DM, (int)DM);
				}

				g.DrawString ("RA : "+Math.Round(planetData.planets["Sun"].skyPosition.RA,3).ToString(), new Font("Arial",10),
					new SolidBrush(Color.Yellow), pB_Space.Width/2+10, 30);
				g.DrawString ("Decl : "+Math.Round(planetData.planets["Sun"].skyPosition.Decl,3).ToString(), new Font("Arial",10),
		...
		{
			if (r_SolarEclipse.Checked == true)
				location.v_mainDT = ((DateTime)dateList[lB_Result.SelectedIndex]).AddMinutes(-90);
			if (r_LunarEclipse.Checked == true)
				location.v_mainDT = ((DateTime)dateList[lB_Result.SelectedIndex]).AddMinutes(-200);
			if (r_MercuryTranzit.Checked == true)
				location.v_mainDT = ((DateTime)dateList[lB_Result.SelectedIndex]).AddHours (-5);
			if (r_VenusTranzit.Checked == true)
				location.v_mainDT = ((DateTime)dateList[lB_Result.SelectedIndex]).AddHours (-5);
			n=0;
			b_Watch.Visible = false;
		...
		private void timer1_Tick(object sender, System.EventArgs e)
		{
			...
			location.v_mainDT = location.v_mainDT.AddMinutes(1);
			planetData.PlanetPositions();
			pB_Space.Invalidate();
		}
